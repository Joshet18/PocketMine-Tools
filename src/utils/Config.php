...
class Config{
	...
	private function load(string $file, int $type = Config::DETECT, array $default = []) : void{
		...
		if($this->type === Config::DETECT){
			$extension = strtolower((new \SplFileInfo($this->file))->getExtension());
		...
		if(!file_exists($file)){
			...
			switch($this->type){
				...
				case Config::YAML:
					...
					$config = self::paser_yaml($content);
					break;
	...
	public function save() : void{
		...
		switch($this->type){
			...
			case Config::ENUM:
				...
				break;
			default:
				throw new \RuntimeException("Config type is unknown, has not been set or not detected");
		}
	...
	public  static function paser_yaml(string $content):array{
		$result = [];
		$data = explode("\n", $content);
